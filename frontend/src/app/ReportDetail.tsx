...
import { fetchReportById, removeReport, clearSelectedReport } from '../store/slices/reportSlice';
import ReportDetailView from '../components/reports/ReportDetailView';
import ConfirmDialog from '../components/common/ConfirmDialog';
...
export default function ReportDetail() {
  ...
  const { selectedReport: report, loading, error } = useAppSelector((state) => state.reports);
  ...
  if (error || !report) return <ErrorMessage message={error || 'Report not found'} />;
  ...
  return (
    <div style={{ maxWidth: '700px', margin: '2rem auto', padding: '0 1rem' }}>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1.5rem', justifyContent: 'flex-end' }}>
        ...
        <button type="button" onClick={() => navigate(`/reports/${id}/edit`)}>
          Edit
        </button>
        <button type="button" onClick={() => setConfirmOpen(true)} style={{ backgroundColor: 'var(--danger)' }}>
          Delete
        </button>
      </div>
